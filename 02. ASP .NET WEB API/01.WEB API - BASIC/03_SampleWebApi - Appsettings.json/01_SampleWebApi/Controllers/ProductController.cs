    ...
    */
    ...
    public class ProductController : ControllerBase
    {
        ...
        */
        ...
        private readonly IConfiguration _configuration;        // 🔹 Direct access
        private readonly MySettings _settings;                  // 🔹 Strongly typed

        public ProductController( IProductService productService, IConfiguration configuration, IOptions<MySettings> options)
        {
        ...
        */
        ...
        #region --- 09. GET APP SETTINGS VALUES ---

        /*
            This method demonstrates:

            ✔ Reading configuration using IConfiguration
            ✔ Reading configuration using IOptions (Strongly Typed)
            ✔ Shows which environment values are currently active
        */

        [HttpGet("config")]
        public IActionResult GetConfigurationValues()
        {
            // 🔹 Direct Reading using IConfiguration
            var appName_Direct = _configuration["MySettings:AppName"];
            var version_Direct = _configuration["MySettings:Version"];
            var connection_Direct = _configuration["ConnectionStrings:DefaultConnection"];

            // 🔹 Strongly Typed Reading using IOptions
            var appName_Typed = _settings.AppName;
            var version_Typed = _settings.Version;

            return Ok(new
            {
                DirectAccess = new
                {
                    AppName = appName_Direct,
                    Version = version_Direct,
                    ConnectionString = connection_Direct
                },
                StronglyTypedAccess = new
                {
                    AppName = appName_Typed,
